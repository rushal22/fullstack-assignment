...
const all_books = async (req,res)=>{
    try {
        const products = await BookService.get_all_books()
...
const add_book = async (req, res) =>{
    try{
        const data = req.body
        console.log(data);
        const result = await BookService.add_books(data.title, data.author, data.isbn, data.price, data.quantity)
...
module.exports = {add_book , remove_book , update_book , search_books , all_books , get_book_by_id}